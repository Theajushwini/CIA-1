...
public class BranchAndBound {
    public static void main(String[] args) {
        ...
        // Call the branchAndBound function
        List<List<Object>> result = branchAndBound(d, "S", "G");
        System.out.println("Final Path: " + result);
    }

    public static List<List<Object>> branchAndBound(Map<String, Map<String, Integer>> d, String start, String end) {
        List<List<Object>> exploredPaths = new ArrayList<>();
        PriorityQueue<List<Object>> queue = new PriorityQueue<>(Comparator.comparingInt(a -> (int) a.get(0)));  // Min-heap priority queue
        int oracle = Integer.MAX_VALUE;  // Start with an infinite oracle (upper bound)
        ...
        while (!queue.isEmpty()) {
            ...
            // If we reached the goal, update the oracle if we found a better path
            if (currentNode.equals(end)) {
                if ((int) currentPath.get(0) < oracle) {
                    oracle = (int) currentPath.get(0);  // Update oracle with the new lower cost
                }
                exploredPaths.add(currentPath);  // Store the path to the goal
                continue;  // Continue to explore more paths
            }
            ...
            for (String neighbor : d.get(currentNode).keySet()) {
                ...
                if (newCost <= oracle) {
                    ...
                    exploredPaths.add(newPath);  // Add to the explored path list
                }
            }
        }
